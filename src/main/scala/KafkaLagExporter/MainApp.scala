...
object MainApp extends App {
  ...
  def start(config: Config = ConfigFactory.load()): ActorSystem[KafkaClusterManager.Message] = {
    ...
    val clientCreator = (cluster: KafkaCluster) => {
      val clientSec = Security(cluster.securityOpt, appConfig.securityOpts)
      KafkaClient(cluster, appConfig.clientGroupId, appConfig.clientTimeout, clientSec)(kafkaClientEc)
    }
    Security.close()
