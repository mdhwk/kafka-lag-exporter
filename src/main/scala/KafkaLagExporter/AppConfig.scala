...
import EndpointSink.ClusterGlobalLabels
...
object AppConfig {
  def apply(config: Config): AppConfig = {
    ...
    val graphiteConfig: Option[GraphiteConfig] = Try(c.getString("reporters.graphite.host"))
      .flatMap(host => {
        Try(c.getInt("reporters.graphite.port"))
          .map(port => GraphiteConfig(
    ...
    val securityOptions = Try {
      val opts = c.getConfig("security-configs")
      opts.entrySet().asScala.map(
        entry => (entry.getKey, entry.getValue.unwrapped().asInstanceOf[Map[String, Map[String, String]]])
      ).toMap[String, Map[String, Map[String, String]]]
    }.getOrElse(Map.empty[String, Map[String, Map[String, String]]])
    val clusters = c.getConfigList("clusters").asScala.toList.map { clusterConfig =>
      ...
      val securityOpt = if (clusterConfig.hasPath("security-option")) {
        clusterConfig.getString("security-config")
      }
      else ""
    ...
    AppConfig(pollInterval, lookupTableSize, clientGroupId, kafkaClientTimeout, clusters, strimziWatcher, metricWhitelist, prometheusConfig, graphiteConfig, securityOptions)
  }
...
final case class AppConfig(pollInterval: FiniteDuration, lookupTableSize: Int, clientGroupId: String,
                           ...
                           securityOpts: Map[String, Map[String, Map[String, String]]]) {
  override def toString(): String = {
    val graphiteString =
