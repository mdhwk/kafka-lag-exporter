...
import Domain.{GroupOffsets, PartitionOffsets}
import KafkaClient.{AdminKafkaClientContract, ConsumerKafkaClientContract, KafkaClientContract}
...
object KafkaClient {
  ...
  def apply(cluster: KafkaCluster, groupId: String, clientTimeout: FiniteDuration, securityProperties: Properties)(implicit ec: ExecutionContext): KafkaClientContract = {
    val consumer = new ConsumerKafkaClient(createConsumerClient(cluster, groupId, clientTimeout, securityProperties), clientTimeout)
    val adminKafkaClient = new AdminKafkaClient(createAdminClient(cluster, clientTimeout, securityProperties), clientTimeout)
  ...
  private def createAdminClient(cluster: KafkaCluster, clientTimeout: FiniteDuration, securityProperties: Properties): AdminClient = {
  ...
  private def createConsumerClient(cluster: KafkaCluster, groupId: String, clientTimeout: FiniteDuration, securityProperties: Properties): KafkaConsumer[Byte, Byte] = {
    val props = new Properties()
